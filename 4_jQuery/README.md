...
- `$('선택자').odd()` : 홀수 번째 인덱스에 위치한 요소 선택
- `$('선택자').even()` : 짝수 번째 인덱스에 위치한 요소 선택
...
- `$('선택자').patents(['선택자])` : 선택된 요소의 모든 상위 요소를 선택하거나, 매개변수가 있으면 매개변수와 일치하는 상위 요소만 선택
...
- 클래스 선택자로 여러 요소가 선택되었을 떄 첫 번째 요소의 텍스트를 리턴하고, 전체 요소의 텍스트 리턴 시 콜백 함수를 사용

```js
$('선택자').html(); // 선택된 요소의 첫 번째 텍스트 리턴
...
- fadeIn() : 선택된 요소를 점점 진하게 보여 주는 메소드
...
- slideDown() : 선택된 요소 위치에서 아래로 요소를 보여 주는 메소드
