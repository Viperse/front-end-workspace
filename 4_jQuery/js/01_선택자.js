$(function() {
    ...
    // 제이쿼리 방식
    $('h5').css('color', 'skyblue').css('background-color', 'red');

    // 2. 아이디 선택자
    // 자바스크립트 방식
    const id1 = document.querySelector('#id1');
    id1.style.color = 'crimson';
    ...
    // option 태그 중 선택된 값이 각각 result1에는 국가명이 들어가고, result2에는 취미가 들어가게
    $('#div3 select').change(function() {
        if(this.id === 'national') {
            $('#result1').val(this.value);
        } else {
            $('#result2').val(this.value);
        };
    })
