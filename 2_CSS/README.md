...
- inline-block : 요소를 인라닌 레벨로 배치하면서 내용에는 블록 레벨 속성을 지정
...
- 무조건 맨위에 표시해야 하는 경우 큰 값으로 지정하기도 한다.
...
```css
.item {
    flex: 1; /* felx-grow: 1; flex-shrink: 1; flex-basis: 0; */
    flex: 1 1 auto; /* flex-grow: 1; fiex-shrink: 1; felx-basis: auto; */
...
- 컨테이너에 Grid 트랙(행 또는 열)의 크기들을 지정해 주는 속성
...
## grid-column-start, grid-column-end, grid-column, grid-row-start, grid-row-end, grid-now
...
```
* 변형함수(3d, X, Y, Z)
- scale : 지정한 크기 만큼 확대/축소 하는 함수
- rotate : 지정한 각도 만큼 회전시키는 함수
- skew : 지정한 각도 만큼 비틀어 왜곡하는 함수
- translate : x축, y축, y축 방향으로 이동시키는 함수
